...
export default async function BarangMasukDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const res = await getBarangMasukDetail(id);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-[10px] border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            <dt className="text-sm text-dark-5 dark:text-dark-6">Supplier</dt>
            <dd className="font-medium text-dark dark:text-white">
              {header.supplierNama || "-"}
            </dd>
          </div>
