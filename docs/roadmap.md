...
- [ ] `app-gtf4.1` — Sambungkan pengirim/penerima jahit ke master (P1) — ✅ **tidak blocked lagi**, semua keputusan lengkap (klien 21 Sep). **Task 1 (migration + schema.ts) sudah dieksekusi Claude via MCP.** Plan+prompt siap, GH #21. 🔜 **ANTRE BERIKUTNYA** setelah Batch B selesai. Prompt disegarkan 21 Sep sore: baseline typecheck 13→**9 error, semuanya milik issue ini, target 0** (app-glx1 sudah selesai jadi error-nya hilang), dan master `users` kini **4 akun di dev + prod** (bukan lagi 1 owner) sehingga dropdown Pengirim/Penerima bisa diuji sungguhan
